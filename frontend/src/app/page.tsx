...
const useProjects = (username: string, token: string) => {
  ...
  useEffect(() => {
    if (!username || !token) {
      // if there's no valid username or token, skip fetching
      setProjects([]);
      setLoading(false);
      return;
    }
...
const HeroSection = () => {
  ...
  // Merge errors (authentication or during project fetching)
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-50 left-8 z-1000">
        ...
        <p className="mt-4 text-2xl text-gray-200">Unleash your creativity with our cutting-edge architectural design platform. Select from a range of room layouts, customize every detail with precision, and visualize your space in stunning 3D. Whether you are an architect, interior designer, or enthusiast, bring your vision to life with seamless real-time rendering and intuitive controls.</p>
